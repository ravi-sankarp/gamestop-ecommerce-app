...
function EditProductOffer({ close, products, productOffer }) {
  const [formError, setFormError] = useState('');
  const [text, setText] = useState('Edit Product Offer');
  ...
  const [editProductOffer, { isLoading }] = useEditOfferMutation();
  const onSubmitHandler = async (data) => {
    if (!isLoading) {
      try {
        setFormError('');
        setText('Adding Offer...');
        const res = await editProductOffer({
          id: productOffer._id,
          data
        }).unwrap();
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        fullWidth
        required
        defaultValue={productOffer.productId.toString()}
        type="text"
        error={!!errors.id}
      ...
      <TextField
        ...
        name="discount"
        type="number"
        defaultValue={productOffer.discount}
